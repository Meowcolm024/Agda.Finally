...
> This part is adapted from [PLFA:Substitution](https://plfa.github.io/Substitution/)

```agda
module stlc.subst where
```

The proofs in this part is almost identical to the one in *PLFA*, please refer back
to *PLFA* for detailed explanation :P.
...
rename-suc-commute {ρ = ρ} {M = M} =
  ...
  ≡⟨ cong (λ x → ⟪ x ⟫ (rename suc M)) (sym ren-ext) ⟩
